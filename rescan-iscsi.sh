...
#
# 
# For ProxMox, disable the iscsid service to make the boot faster
# script will start with crontab each 1 minute
# Auto start the iscsid if needed, rescan the iscsi targets as soon they are available
#
...
if [[ "$(systemctl is-enabled iscsid)" == "enabled" ]]; then systemctl disable iscsid ;fi
